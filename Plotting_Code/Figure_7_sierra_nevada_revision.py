...
path = 'C:/Users/Stuart/Dropbox/LH_Paper/cr/' #path to the folder contaning the hilltopdata files
filename = 'CR2_PaperData.txt'#names of the hilltopdata file
...
for subplot_count, (Method,xmax,ymax,xstep,ystep,labels,v_line_lim) in enumerate(zip(Methods,xmaxes,ymaxes,xsteps,ysteps,fig_labels,v_line_lims)):
    ...
    if subplot_count == 0:
        ax.text(.55,.9,Method, horizontalalignment='center', transform=ax.transAxes,fontsize=12)
    else:
...
# title
fig.text(0.5, 0.925, 'Sierra Nevada', ha='center', va='center', size=14)
