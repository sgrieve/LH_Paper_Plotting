...
trace_files = ['Traces_Aspect.shp','Traces_Dinf.shp']
labels = ['a','b']
...
for i in [0,1]:
    ...
    plt.annotate(labels[i], xy=(0.92, 0.96), backgroundcolor='white', xycoords='axes fraction', fontsize=12, horizontalalignment='left', verticalalignment='top')

    x_center = int(x_max/2.)    
    y_center = int(y_max/2.)    
       
    #load trace shapefile
    sf = shp.Reader(data_path + trace_files[i])
...
plt.savefig('Figure_4.png', dpi = 500) #change to *.tif for submission
