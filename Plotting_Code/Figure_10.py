...
Script to generate Figure 10 from Grieve et al. (2015)
...
def LH_Rel(k,Sc):
    ...
    we have the values of all of the independent avraibels so we want to create a series of numpy arrays
    of the same dimensions as LH, filled with the parameter values which have been published.
    
    This FN is implementing equation X from Grieve et al 2015, based on work in Roering 2007
    """
...
DDs = [0.005,0.003,0.038,0.0086]
EEs = [0.00027,0.1,0.00036,0.25]

#plot style parameters
xmaxes = [200,200,300,400]
ymaxes = [100,200,200,300]
xsteps = [50,100,100,100]
ysteps = [50,100,100,100]
...
for subplot_count, (filename,DD,EE,fig_label,location,xmax,ymax,xstep,ystep) in enumerate(zip(Filenames,DDs,EEs,fig_labels,locations,xmaxes,ymaxes,xsteps,ysteps)):
    ...
    #add in the subplot label
    plt.annotate(fig_label, xy=(0.95, 0.95), xycoords='axes fraction', fontsize=10, horizontalalignment='left', verticalalignment='top')
...
plt.savefig(figpath + 'Figure_10.png', dpi = 500) #change to *.tif for submission